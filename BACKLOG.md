...
- Sıradaki tek adım: **ÜCRETSİZ mode solver testi.** `freq-rung-1` tamamlandı (`11.5346 FC`): monitör düzeltmesi çalıştı (FWHM başına `79.8` nokta), çizgi çözüldü, `Q_loaded ~ 9 650` (drop `9 736` / through `9 557`), FWHM `~0.159 nm`, extinction `13.56 dB`. Ring-down'dan çıkardığım `~27 500` çürütüldü (`2.8x` yüksekti). AMA `Q_i`/`Q_e` ayrımı GEÇERSİZ: `T_through = (1-sqrt(T_drop))^2` tutarlılık koşulu tutmuyor (`0.403` vs `0.177`) çünkü enerji dengesi kapanmıyor — rezonans dışı `%12-23`, rezonansta `%69` ölçülmeyen; malzemeler kayıpsız (`conductivity=0`) olduğu için bu emilim olamaz. Üç neden: (a) `flux:bus_drop:out` `x=+8`'de, yani ADD portunda — eş-yönlü add-drop halkada gerçek drop portu `bus_drop` üzerinde `x=-8`; ayrıca `bus_through` `x=-8` geri-yansıma portu izlenmiyor (dört portun ikisi ölçülmüyor). (b) buried oxide `1.0 um` ve altında Si substrate var; 450x220 nm waveguide için sızıntıya açık (tipik SOI 2-3 um) — HİPOTEZ. (c) drop tepesi ile through çukuru `46 pm` ayrı (FWHM'in %29'u), backscattering/Fano şüphesi. Sıra: (1) ÜCRETSİZ mode solver ile `1.0/2.0/3.0 um` BOX taraması -> sızıntı hükmü; (2) ÜCRETSİZ dört-port monitör tokenları + enerji dengesi testi; (3) ücretli tek koşu ile `Q_i`/`Q_e`; (4) ancak ondan sonra 14 step/λ mesh yakınsaması. Bkz. `docs/decisions/2026-09-13-freq-rung-1-evaluation.md`.
